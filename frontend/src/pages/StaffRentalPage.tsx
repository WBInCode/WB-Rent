...
export function StaffRentalPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (form.renterAddress.trim().length < 5) {
      setError('Adres zamieszkania musi mieć co najmniej 5 znaków.');
      return;
    }
    if (form.documentNumber.trim().length < 3) {
      setError('Numer dokumentu musi mieć co najmniej 3 znaki.');
      return;
    }
    if (!/^[\p{L}\d\s-]+$/u.test(form.documentNumber.trim())) {
      setError('Numer dokumentu może zawierać tylko litery, cyfry, spacje i myślniki.');
      return;
    }
    if (form.pesel && !/^\d{11}$/.test(form.pesel)) {
      setError('PESEL musi składać się dokładnie z 11 cyfr albo pozostać pusty.');
      return;
    }
    if (form.employeeName.trim().length < 3) {
      setError('Podaj imię i nazwisko pracownika wydającego.');
      return;
    }
    if (form.accessories.trim().length < 2) {
      setError('Wpisz wydawane akcesoria lub informację „brak”.');
      return;
    }
    if (form.conditionNotes.trim().length < 2) {
      setError('Opisz stan sprzętu przy wydaniu.');
      return;
    }
    setSubmitting(true);
    setError('');
              ...
              <FormCard number="2" icon={<User className="w-5 h-5" />} title="Dane klienta" description="Przepisz dane dokładnie z dokumentu tożsamości.">
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <Input
                    ...
                    maxLength={11}
                    pattern="[0-9]{11}"
                    hint="11 cyfr albo pozostaw puste"
                  />
                      ...
                      {price && (
                        <div className="mt-5 pt-5 border-t border-border space-y-2.5 text-sm">
                          ...
                          <div className="flex justify-between items-end pt-3 mt-3 border-t border-border">
                            ...
                            <span className="text-3xl font-bold text-gold">{price.total} zł</span>
                          </div>
